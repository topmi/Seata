...
JAVA_OPT="${JAVA_OPT} -server -Xmx${JVM_XMX:="1024m"} -Xms${JVM_XMS:="1024m"} -Xmn${JVM_XMN:="512m"} -Xss${JVM_XSS:="256k"} -XX:SurvivorRatio=10 -XX:MetaspaceSize=${JVM_MetaspaceSize:="64m"} -XX:MaxMetaspaceSize=${JVM_MaxMetaspaceSize:="128m"} -XX:MaxDirectMemorySize=${JVM_MaxDirectMemorySize:=512m} -XX:-OmitStackTraceInFastThrow -XX:-UseAdaptiveSizePolicy"
